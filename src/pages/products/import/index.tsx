      ...
      try {
        ...
        await variations.forEach(async (variation: VariationDTO, i: number) => {
          if (!!product.grouperId && product.grouperId !== '') {
            ...
            await api.patch(
              `/product/${product._id}/variation/${variationId}`,
              variation,
              {
                headers: {
                  authorization: token,
        ...
        await api
          .patch(`/product/${product._id}`, product, {
            headers: {
              authorization: token,
              shop_id: user.shopInfo._id,
            },
          ...
          .then(() => {
            ...
            handleModalMessage(true, {
              ...
              message: [
                `Foram alterados ${products.length} produtos e suas variações`,
              ],
              type: 'success',
            });
    ...
    async (imprts: ProductImport[]) => {
      ...
      try {
        ...
        products.map(async product => {
          ...
          const nationality =
            nationalityIndex > -1
              ? nationalities[nationalityIndex]
              : nationalities[0];
          ...
          const category =
            categoryIndex > -1 ? categories[categoryIndex] : categories[0];

          const subCats = await api
            .get(`/category/${category.code}/subcategories`)
            .then(response => response.data as SubCategory[])
            .catch(err => []);

          const subCategoryIndex = subCats.findIndex(
            ...
            await api
              ...
              .then(response => {
                handleModalMessage(true, {
                  ...
                  message: [
                    `Foram cadastrados ${products.length} produtos e suas variações`,
                  ],
                  type: 'success',
                });
              })
        ...
        if (!!result.Planilha1 || !!result.data) {
            ...
            if (i > 1 && line.length > 0) {
              ...
              if (!line[3]) {
                handleModalMessage(true, {
                  ...
                  message: [
                    `Não foi encontrado um id arupador na linha ${i + 1}`,
                  ],
                });
          ...
          if (count === 0) {
            handleModalMessage(true, {
              ...
              message: [
                'A planilha selecionada não possui nenhum registro válido',
              ],
            });
    ...
    <>
      <div className={styles.importContainer}>
        <section className={styles.importHeader}>
          ...
          <BulletedButton isActive>Importar ou exportar</BulletedButton>
        </section>
        ...
        <section className={styles.importContent}>
          <div className={styles.exportPanel}>
            ...
            <p>
              A planilha inicial é um arquivo com todos os campos que você
              precisa preencher para realizar a importação.
            </p>
            {/* <button type='button'> */}
            <a href="/assets/CadastroProduto.xlsx" target="_blank" download>
              Exportar Planilha
            </a>
            {/* </button> */}
          </div>
          <div className={styles.importPanel}>
            ...
            <p className={styles.smallText}>
              São aceitas planilhas no formato *.xlsx, *.xls e *.csv com tamanho
              de até 10MB
            </p>
            <Form
              ref={formRef}
