  ...
  const handleSubmit = useCallback(async (data: Product) => {
    try {
      ...
      const schema = Yup.object().shape({
        images: Yup.array().required('Escolha pelo menos \numa imagem').min(1, 'Escolha pelo menos \numa imagem'),
        ...
        variations: Yup.array().required().of(Yup.object().shape({
          ...
          stock: Yup.number().required('Campo obrigatório').min(0, 'Valor mínimo 0'),
        })),
    ...
    <div className={styles.wrapper}>
      <main className={styles.container}>
        ...
        <section className={styles.content}>
          <Form ref={formRef} onSubmit={() => { }} onChange={(e) => {
