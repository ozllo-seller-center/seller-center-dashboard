...
import {
  FiAlertTriangle, FiCalendar, FiCheck, FiClipboard, FiMoreHorizontal, FiPaperclip, FiSearch, FiX,
} from 'react-icons/fi';
...
export const SellsMobile: React.FC = () => {
  ...
  const collapsibleRefs = useMemo(() => items.length > 2 && Array(items.length).fill(0)
    .map(() => React.createRef<HTMLDivElement>()), [items]);
  ...
  useEffect(() => {
    ...
    setItems(newItems);
  }, [orders, status, orderStatus, fromDateFilter, toDateFilter, search, filter, inInterval]);
